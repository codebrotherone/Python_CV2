...
def draw_tl_center(image_in, center, state):
    ...
    """
    img= image_in.copy()
    cv2.putText(img, "( " + str(center) + ", '" + state + "'" + ')', org=center ,
                fontFace=cv2.FONT_HERSHEY_COMPLEX_SMALL, fontScale=.5, color=(0,0,0))
...
def part_2():
    ...
    input_images = [ 'scene_yld_1'] #'scene_dne_1', 'scene_stp_1', 'scene_constr_1', 'scene_wrng_1',

    output_labels = ['ps2-2-a-5'] #'ps2-2-a-1', 'ps2-2-a-2', 'ps2-2-a-3', 'ps2-2-a-4',

    sign_fns = [ps2.yield_sign_detection] #ps2.do_not_enter_sign_detection, ps2.stop_sign_detection, ps2.construction_sign_detection, ps2.warning_sign_detection,

    sign_labels = ['yield'] #'no_entry', 'stop', 'construction', 'warning',
